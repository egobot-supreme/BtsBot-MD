...
let creds = await fs.readFileSync(`./BtsBotSession/creds.json`)
...
case isCommand3:
if (m.fromMe) return
if (isAdmin) throw lenguajeEB.smsAutoAdmin1()
try {  
...
case isCommand4:
if (!text) throw lenguajeEB.smsBioEd1()
if (text.length > 139) throw lenguajeEB.smsBioEd2()
try {
...
case isCommand5:
if (!text) throw lenguajeEB.smsNameEd1()
if (text.length > 25) throw lenguajeEB.smsNameEd2()
try {
await conn.updateProfileStatus(text).catch(_ => _)
...
if (!img) throw  lenguajeEB.smsFotoEd1(usedPrefix, command)
await conn.updateProfilePicture(bot, img)
await conn.reply(m.chat, lenguajeEB.smsFotoEd2(), m)
} else throw lenguajeEB.smsFotoEd3(usedPrefix, command)        
break
...
if (!code1) throw lenguajeEB.smsJoin1(usedPrefix, command)
try {      
if ( isOwner || m.fromMe) {
...
let año = d.toLocaleDateString(lenguajeEB.lenguaje(), { year: 'numeric' })
...
let infoGP = lenguajeEB.smsChatGP2(readMS, dia, mes, año, fecha, tiempo)  
